...
<data>
	<features>
		<?php while($feature = $featureSearch->nextResult()) { ?>
			<feature id="<?php echo $feature->getId() ?>" lat="<?php echo $feature->getPointLat() ?>" lng="<?php echo $feature->getPointLng() ?>" title="<?php echo htmlspecialchars($feature->getTitle(),ENT_QUOTES,'UTF-8') ?>" <?php if ($user) { ?>answeredAllQuestions="<?php echo  $feature->getHasUserAnsweredAllQuestions() ? 'yes' : 'no' ?>"<?php } ?>>
